...
public final class DBConnection {
    ...
    /**
     * SQLite file is created in the working directory (NetBeans: project folder).
     */
    ...
    /**
     * True for duplicate key / unique (MySQL 1062 or SQLite constraint).
     */
    public static boolean isUniqueConstraintViolation(SQLException e) {
        if (e.getErrorCode() == 1062) {
            return true;
        }
        if (e.getErrorCode() == 19 || e.getErrorCode() == 2067) {
            return true;
        }
        String m = e.getMessage();
        if (m != null) {
            String low = m.toLowerCase();
            return low.contains("unique constraint failed") || low.contains("unique constraint");
        }
        return false;
    }
    ...
    public static String userMessage(SQLException e) {
        ...
        if (lower.contains("no such table")
            ...
            sb.append("SQLite database problem.\n\n");
            sb.append("Try this:\n");
            sb.append("• Check db.properties: db.url (e.g. jdbc:sqlite:qualimed_pharmacy.db)\n");
            sb.append("• Ensure the app can write the folder (the .db file is created there).\n");
            sb.append("• Delete a corrupted qualimed_pharmacy.db and run the app again to recreate tables.\n\n");
            sb.append("Details: ").append(msg);
            return sb.toString();
        }
        return msg;
    }
